...
///  * <https://material.io/design/components/lists.html#types>
class ExpansionPanelListFixed extends StatefulWidget {
  /// Creates an expansion panel list widget. The [expansionCallback] is
  /// triggered when an expansion panel expand/collapse button is pushed.
  ///
  /// The [children] and [animationDuration] arguments must not be null.
  const ExpansionPanelListFixed({
    Key key,
    this.children = const <ExpansionPanelFixed>[],
  ...
  /// ```
  /// {@end-tool}
  const ExpansionPanelListFixed.radio({
    Key key,
    this.children = const <ExpansionPanelRadioFixed>[],
    this.expansionCallback,
    this.animationDuration = kThemeAnimationDuration,
  ...
  /// fashion to [ListBody].
  final List<ExpansionPanelFixed> children;
...
class _ExpansionPanelListState extends State<ExpansionPanelListFixed> {
  ExpansionPanelRadioFixed _currentOpenPanel;
  ...
  void initState() {
    ...
    if (widget._allowOnlyOnePanelOpen) {
      ...
      if (widget.initialOpenPanelValue != null) {
        _currentOpenPanel =
            searchPanelByValue(widget.children.cast<ExpansionPanelRadioFixed>(), widget.initialOpenPanelValue);
      }
    }
  }

  @override
  void didUpdateWidget(ExpansionPanelListFixed oldWidget) {
  ...
  bool _allIdentifiersUnique() {
    ...
    for (final ExpansionPanelRadioFixed child in widget.children.cast<ExpansionPanelRadioFixed>()) {
  ...
  bool _isChildExpanded(int index) {
    if (widget._allowOnlyOnePanelOpen) {
      final ExpansionPanelRadioFixed radioWidget = widget.children[index] as ExpansionPanelRadioFixed;
  ...
  void _handlePressed(bool isExpanded, int index) {
    ...
    if (widget._allowOnlyOnePanelOpen) {
      final ExpansionPanelRadioFixed pressedChild = widget.children[index] as ExpansionPanelRadioFixed;

      // If another ExpansionPanelRadio was already open, apply its
      // expansionCallback (if any) to false, because it's closing.
      for (int childIndex = 0; childIndex < widget.children.length; childIndex += 1) {
        final ExpansionPanelRadioFixed child = widget.children[childIndex] as ExpansionPanelRadioFixed;
        if (widget.expansionCallback != null &&
            childIndex != index &&
  ...
  ExpansionPanelRadioFixed searchPanelByValue(List<ExpansionPanelRadioFixed> panels, Object value)  {
    for (final ExpansionPanelRadioFixed panel in panels) {
  ...
  Widget build(BuildContext context) {
    ...
    for (int index = 0; index < widget.children.length; index += 1) {
      ...
      final ExpansionPanelFixed child = widget.children[index];
